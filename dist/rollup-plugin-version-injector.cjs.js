...
class VIInjector {
    ...
    writeToFile(outputFile) {
        const finalFile = path__default['default'].resolve(process.cwd(), outputFile);
        fs__default['default'].writeFileSync(finalFile, this.code);
...
class VILogger {
    ...
    logger(level, ...args) {
        if ((this.logMap.get(level) || this.DEFAULT_LOG_LEVEL) >= this.logLevel) {
            this._logger[level](chalk__default['default'].magenta.bold('  [VI]'), chalk__default['default'].yellow(`[${level.padEnd(5)}]`), ...args);
        }
