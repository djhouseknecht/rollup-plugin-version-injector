...
describe('VIInjector', () => {
	...
	describe('injectIntoTags()', () => {
		test('should log an error and return if there is no code on the class instance', () => {
			const config = { fileRegexp: /./, tagId: 'VI', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
		...
		test('should log a debug message if config fileRegexp does not match', () => {
			const config = { fileRegexp: /\.(js)$/g, tagId: 'VI', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
			const code = 'var = 12;';
			injector.setCode(code);
	...
	describe('injectIntoComments()', () => {
		test('should log an error and return if there is no code on the class instance', () => {
			const config = { fileRegexp: /./, tag: '{version}', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
		...
		test('should log a debug message if config fileRegexp does not match', () => {
			const config = { fileRegexp: /\.(js)$/g, tag: '{version}', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
			const code = 'var = 12;';
			injector.setCode(code);
		...
		test('should log a warn message if file extension is not supported', () => {
			const config = { fileRegexp: /\.(cpp)$/g, tag: '{version}', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
			const code = 'var = 12;';
			injector.setCode(code);
	...
	describe('replaceVersionInTags()', () => {
        ...
        appName: 'some cool app'
			...
			const tagId = 'VI';
			const dateFormat = 'mmmm d, yyyy HH:MM:ss';
			const version = '7.1.1';
			const expectedCode = `{
