...
describe('VIInjector', () => {
  ...
  describe('injectIntoTags()', () => {
    ...
    test('should log a debug message if config fileRegexp does not match', () => {
      const config = { fileRegexp: /\.(js)$/, tagId: 'VI', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
      const code = 'var = 12;';
      injector.setCode(code);
  ...
  describe('injectIntoComments()', () => {
    ...
    test('should log a debug message if config fileRegexp does not match', () => {
      const config = { fileRegexp: /\.(js)$/, tag: '{version}', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
      const code = 'var = 12;';
      injector.setCode(code);
    ...
    test('should log a warn message if file extension is not supported', () => {
      const config = { fileRegexp: /\.(cpp)$/, tag: '{version}', dateFormat: 'mmmm d, yyyy HH:MM:ss' };
      const code = 'var = 12;';
      injector.setCode(code);
